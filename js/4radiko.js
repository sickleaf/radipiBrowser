...
function playStreamURL (val) {
	...
	document.getElementById(Radipi.nowplayingID).innerHTML=val;
        doCommand(Radipi.sleepCommand + " && " + Radipi.scriptPath + Radipi.streamingScript + " " +  val );
}
...
function playfromStreamList(streamID){
	...
	var streamValue = streamList.options[streamIndex].value;

	playStreamURL(streamValue);
}
